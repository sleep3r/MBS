...
class Loan_Portfolio:
        ...
        for i in range(1, 361):
            ...
            for j, loan_row in loan_df.iterrows():
                ...
                if period_num_1 == 1:
                    ...
                    last_period = tranchwise_df.loc[
                        (tranchwise_df['period'] == (i - 1)) & (tranchwise_df['loan_df_pk'] == loan_pk_num)]

                # last_period=tranchwise_df.loc[(tranchwise_df['loan_pk_num']==j )& (tranchwise_df['period'] == (i+1))]
                if i < rem_term_t:
                    p = create_payment_schedule_period(loan_pk_num, float(last_period['end_balance']),
                                                       float((last_period['remaining_term'] - 1)),
                                                       cdr_t, cpr_t, period_num_1, int_rate_t, recov_percent_t, N_t,
                                                       tranche_t, principal)
            ...
            if senior_tranch_outstanding_bal[0] > 0 and senior_tranch_outstanding_bal[0] > junior_tranch_prepayment[1]:
                ...
                if (periodwise_df.loc[(periodwise_df['tranche'] == 1), 'end_balance'] > prep_per_loan).all():
                    ...
                    periodwise_df.loc[(periodwise_df['tranche'] == 2), 'add_prepayment'] = 0
                    periodwise_df.loc[
                        ((periodwise_df['tranche'] == 1) & periodwise_df['end_balance'] < 0, 'end_balance')] = 0
                    ...
                    num_of_loans = (
                            (periodwise_df['tranche']) == 1 & (periodwise_df['end_balance'] > prep_per_loan)).sum()
                    prep_per_loan = junior_tranch_prepayment[1] / num_of_loans
                    periodwise_df.loc[
                        ((periodwise_df['tranche'] == 1) & periodwise_df['end_balance'] < 0, 'end_balance')] = 0
                    periodwise_df.loc[
                        ((periodwise_df['tranche'] == 1) & periodwise_df['end_balance'] < 0, 'add_prepayment')] = 0
                    periodwise_df.loc[((periodwise_df['tranche'] == 1) & periodwise_df['end_balance'] > prep_per_loan,
                                       'add_prepayment')] = prep_per_loan
                    periodwise_df.loc[(periodwise_df['tranche'] == 2), 'add_prepayment'] = 0
                    periodwise_df.loc[(periodwise_df['tranche'] == 2), 'prepayment'] = 0
                    junior_tranch['prepayment'] = 0
            ...
            if senior_tranch_defaults[0] > junior_tranch_outstanding_bal[1].any():
                ...
                if (periodwise_df.loc[(periodwise_df['tranche'] == 2), 'end_balance'] > def_per_loan).all():
                    ...
                    default_num = (
                        (periodwise_df.loc[(periodwise_df['tranche'] == 2), 'end_balance'] > def_per_loan)).sum()
                    def_per_loan = senior_tranch_defaults[0] / default_num
                    periodwise_df.loc[((periodwise_df['tranche'] == 2) & (periodwise_df['end_balance'] > def_per_loan),
                                       'add_default')] = def_per_loan
                    periodwise_df.loc[(
                        (periodwise_df['tranche'] == 2) & (periodwise_df['end_balance'] < def_per_loan),
                        'add_default')] = 0
                    periodwise_df.loc[(periodwise_df['tranche'] == 1), 'add_default'] = 0
                else:
                    periodwise_df.loc[(
                        (periodwise_df['tranche'] == 2) & periodwise_df['end_balance'] < def_per_loan, 'add_default')] = \
                        periodwise_df.loc[(periodwise_df['tranche'] == 2), 'end_balance']
                    # adjust this  in tranch 1
            periodwise_df['prepayment'] += periodwise_df['add_prepayment']
            periodwise_df['end_balance'] = periodwise_df['end_balance'] - periodwise_df['add_prepayment'] - \
                                           periodwise_df['add_default']
    ...
    def total_principals_for_cash_flow(self):
        total_principals = self.cashflows_dataframe['scheduled_principal'] + self.cashflows_dataframe['prepayment'] + \
                           self.cashflows_dataframe['recovery']
    ...
    def cash_flow_tranchwise(self, loan_df):
        ...
        tranch_senior_outstanding_bal = (tranch_senior_df.groupby(['tranch_level'])[
                                             'Current_Principal_Balance'].sum(lambda x: tuple(x)).reset_index())[
        ...
        WAC_senior_tranch = ((tranch_senior_df['Current_Interest_Rate'] * tranch_senior_df[
            'Current_Principal_Balance']).sum()) / tranch_senior_outstanding_bal
        WAC_junior_tranch = ((tranch_junior_df['Current_Interest_Rate'] * tranch_junior_df[
            'Current_Principal_Balance']).sum()) / tranch_junior_outstanding_bal
        recovery_senior_rate = ((tranch_senior_df['adjusted_recovery'] * tranch_senior_df[
            'Current_Principal_Balance']).sum()) / tranch_senior_outstanding_bal
        recovery_junior_rate = ((tranch_junior_df['adjusted_recovery'] * tranch_junior_df[
            'Current_Principal_Balance']).sum()) / tranch_junior_outstanding_bal
        ...
        while j < N:
            ...
            if ((
                    ...
                    and tranch_senior_outstanding_bal > (
                            prepayment_senior + prepayment_junior + principal_tranch_junior + principal_tranch_senior)):
        ...
        ab = final_df['cashflow_junior'].sum() / (tranch_junior_df.groupby(['tranch_level'])[
            ...
            lambda x: tuple(x)).reset_index())['Current_Principal_Balance'][0]
